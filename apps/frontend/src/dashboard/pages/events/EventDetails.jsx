...
// ── Donations tab ──────────────────────────────────────────────
function DonationsTab({ donations, canManage, onAdd, onDelete }) {
  ...
  const columns = [
    ...
    ...(canManage ? [{ key: 'actions', title: '', align: 'right', render: (_, row) => (
      ...
      <Card>
        <CardBody>
          <EmptyState
            ...
            action={canManage ? <Button icon={Plus} onClick={onAdd}>Record Donation</Button> : null}
          />
        </CardBody>
      </Card>
    );
    ...
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader
          ...
          action={canManage ? <Button size="sm" icon={Plus} onClick={onAdd}>Record Donation</Button> : null}
        />
        <CardBody className="p-0">
...
// ── Expenses tab ───────────────────────────────────────────────
function ExpensesTab({ expenses, canManage, onAdd, onDelete }) {
  ...
  return (
    <Card>
      <CardHeader
        ...
        action={canManage ? <Button size="sm" icon={IndianRupee} onClick={onAdd}>Add Expense</Button> : null}
      />
      <CardBody className="p-0">
        {expenses.length === 0 ? (
          <div className="p-6">
            <EmptyState
              ...
              action={canManage ? <Button icon={Plus} onClick={onAdd}>Add Expense</Button> : null}
            />
          </div>
        ) : (
          <div className="divide-y divide-sand-100 dark:divide-neutral-800">
...
export default function EventDetails() {
  ...
  const canManage = hasPermission('events.update'); // edit / record donation / add & delete
  ...
  const tabs = [
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card className="md:col-span-2">
            ...
            <CardBody>
              ...
              <div className="grid grid-cols-2 gap-4 mt-6">
                {[
                  { l: 'Property', v: event.category || '—', i: MapPin },
          ...
          <Card>
            ...
            <CardBody>
              ...
              {canManage && <Button fullWidth className="mt-5" icon={HandHeart} onClick={() => setDonationModal(true)}>Record Donation</Button>}
            </CardBody>
          </Card>
        </div>
      ),
    },
    {
      ...
      content: <ExpensesTab expenses={expenses} canManage={canManage} onAdd={() => setExpenseModal(true)} onDelete={deleteExpense} />,
    },
    {
      key: 'donations',
      label: 'Donations',
      ...
      content: <DonationsTab donations={donations} canManage={canManage} onAdd={() => setDonationModal(true)} onDelete={deleteDonation} />,
    },
    {
      key: 'gallery',
      label: 'Gallery',
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-3xl h-64 md:h-80">
        {event.image && <img src={resolveMedia(event.image)} onError={onImgError} alt={event.title} className="w-full h-full object-cover" />}
