...
export default function ActivitySummary() {
  return (
    <Card>
      <CardHeader title="Temple Activity" subtitle="This week summary" />
      <CardBody>
          ...
          {activitySummary.map((a, i) => {
            ...
            return (
              <div key={a.label} className="relative p-4 rounded-xl bg-gradient-to-br from-sand-50 to-white dark:from-neutral-800/40 dark:to-neutral-900 border border-sand-200/60 dark:border-neutral-800 overflow-hidden">
                <div className={cn('absolute -top-4 -right-4 w-16 h-16 rounded-full bg-gradient-to-br opacity-10', tones[i])} />
                <div className={cn('w-9 h-9 rounded-lg bg-gradient-to-br flex items-center justify-center mb-2', tones[i])}>
                ...
                <p className={cn('text-[11px] font-medium', positive ? 'text-emerald-600' : 'text-rose-600')}>
                  {positive ? '+' : ''}{a.growth}% vs last week
                </p>
              </div>
            );
          })}
