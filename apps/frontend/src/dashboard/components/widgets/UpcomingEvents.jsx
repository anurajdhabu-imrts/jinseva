            ...
            <div key={e.id} className="flex gap-3 p-4 hover:bg-sand-50/50 dark:hover:bg-neutral-800/30 transition-colors">
              <div className="shrink-0 w-14 text-center">
                <div className="rounded-xl bg-gradient-to-br from-saffron-500 to-maroon-600 text-white py-2">
                  <div className="text-[10px] uppercase tracking-wider font-bold opacity-90">{new Date(e.date).toLocaleDateString('en-IN', { month: 'short' })}</div>
                  <div className="text-xl font-serif font-bold leading-none">{new Date(e.date).getDate()}</div>
                </div>
              </div>
